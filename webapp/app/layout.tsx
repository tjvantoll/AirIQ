...
export const metadata: Metadata = {
  title: "AirIQ",
  description: "Air quality monitoring for the AirIQ connected product.",
  icons: { icon: "/logo.png" },
};
      ...
      <body className="flex min-h-full flex-col font-sans">
        <header className="sticky top-0 z-20 border-b border-line bg-[var(--bg)]/85 backdrop-blur">
          <div className="mx-auto flex max-w-6xl items-center justify-between gap-4 px-4 py-3 sm:px-6">
            <Link href="/" className="shrink-0" aria-label="AirIQ home">
              <Logo />
            </Link>
            <NavLinks />
          </div>
        ...
        <footer className="border-t border-line px-4 py-5 text-center text-xs text-muted sm:px-6">
          ...
          . Readings are plotted on capture time, not upload time.
        </footer>
      </body>
    </html>
  );
